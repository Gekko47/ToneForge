...
Installed [`husky 9.1.7`](../package.json:65) replaced `.husky/_/husky.sh` (generated, never committed) with a deprecation stub that only echoes:
...
1. Git uses `core.hooksPath` from local `.git/config` pointing at `_` (`.husky/_`).
2. Dispatcher `.husky/_/h` runs `sh -e` on the user hook.
...
New v9/v10 architecture confirmed in the installed `node_modules/husky/index.js`
(setup function at line 8; `_` generated at lines 18-22):

- `git config core.hooksPath` set to `.husky/_` (line 14)
- `_/.gitignore` contains `*` (line 20), so `_` is generated and never committed
- `_/h` is the generated dispatcher: a copy of the package source file
  `node_modules/husky/husky` (line 21). The two are not the same file — one is
  the source that ships in the package, the other is the per-clone copy Git
  actually executes.
- Per-hook shims in `_` source `_/h` (line 22)
...
| 5   | `HUSKY=0` skip in the dispatcher (`node_modules/husky/husky`, line 14) and `HUSKY=2` debug branch (same file, line 2) | No change announced; keep relying on it                                    | No action; document `HUSKY=0` as the supported bypass                                                                                                                                                                         |
...
- [`package-lock.json`](../package-lock.json:41) regenerated; the generated `.husky/_` dispatcher regenerated via `prepare`
...
2. **Regenerate:** `npm run prepare` completed successfully; local `.git/config` remains `core.hooksPath = .husky/_`, and the generated `.husky/_/.gitignore` remains `*`.
...
- Do not edit the generated `.husky/_` files by hand — always regenerate via `prepare`.
