...
Actions: fix `developer.contactUrl/supportUrl` to URLs, set `webApplicationInfo.id` to GUID or remove SSO block if unused, reconcile unified vs XML schema via Microsoft Learn, fix [`validate-manifest.mjs`](scripts/validate-manifest.mjs:23) version comparison to string `1.10`, add XML fallback note. Validate with `office-addin-manifest validate`.
Validation: `npm run validate`, `npm run build`, sideload smoke test.
Completion: validator passes, sideload succeeds on web.

### P1 — Correctness and contracts

**R4 — Harden domain schemas and factories**
Actions: add `refine(start <= end)` to [`RangeSchema`](src/core/domain/Finding.ts:14) and Change range, require `payload.text` for insert/replace via discriminated union or `superRefine`, fix `emDash` enum, replace `crypto.randomUUID()` with `uuid.v4()` from [`package.json`](package.json:39), export all schemas from [`core/domain/index.ts`](src/core/domain/index.ts:1). Add Finding, Change, ChangePlan unit tests including invalid UUID, bad dates, empty hash.
Validation: `npm run typecheck`, `npm run test -- tests/unit/core/domain/`.
Completion: invalid ranges and payloads rejected, all schemas exported and tested.
