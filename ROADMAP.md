...
| 2          | Capability evidence tiers, verified Word paragraph events, and B23 workflow projection                                                     | IN PROGRESS | [`capabilityEvidence.ts`](src/word/capabilityEvidence.ts) defines API-present/host-tested/release-supported tiers, [`wordParagraphEvents.ts`](src/word/wordParagraphEvents.ts) registers and removes WordApi 1.6 paragraph added/changed/deleted handlers with identifier normalization and conservative fallback, and [`workflowState.ts`](src/taskpane/workflow/workflowState.ts) provides the B23 projection store. Live host event evidence and full view migration remain open. |
