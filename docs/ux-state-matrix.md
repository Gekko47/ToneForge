...
| Consistency review entry | AI Review page                                               | `Consistency Review (optional)` section with a whole-document-only button, disabled with an adjacent hint when consent or a provider is missing | Open Settings from the hint, otherwise the review | The consistency run             | `ConsistencyReviewEntry`; the engine's own consent gate refuses a second time |
